...
const validation_1 = require("../src/events/validation");
describe("validateCreateEvent", () => {
    ...
    it("returns ok for valid payload", () => {
        const result = (0, validation_1.validateCreateEvent)(validPayload);
    ...
    it("returns errors for non object", () => {
        const result = (0, validation_1.validateCreateEvent)(null);
    ...
    it("returns errors for missing fields", () => {
        const result = (0, validation_1.validateCreateEvent)({});
        expect(result.ok).toBe(false);
        if (!result.ok) {
            expect(result.errors).toEqual(expect.arrayContaining(["Le titre est requis.", "Le contenu est requis.", "L'adresse est requise."]));
        }
    });
    ...
    it("returns errors for non-numeric coordinates", () => {
        ...
        expect(result.ok).toBe(false);
        if (!result.ok) {
            expect(result.errors).toEqual(expect.arrayContaining(["La latitude doit être un nombre.", "La longitude doit être un nombre."]));
        }
    });
