<template>
  <section class="grid gap-6">
    <div class="rounded-[1.75rem] border border-sky-100 bg-[linear-gradient(135deg,rgba(240,249,255,0.96),rgba(255,255,255,0.98))] p-6 shadow-[0_24px_60px_-38px_rgba(15,23,42,0.24)]">
      ...
      <h2 class="mt-3 text-2xl font-semibold text-slate-950">{{ t("admin.categoriesTitle") }}</h2>
        ...
        <div class="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.22)]" data-testid="admin-category-form">
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <h3 class="mt-2 text-lg font-semibold text-slate-950">
                {{ adminCategoryEditingId ? t("admin.editCategory") : t("admin.createCategory") }}
              </h3>
            </div>
            <button
          ...
          <div class="mt-5 grid gap-4">
            <label class="text-sm text-slate-600">
              {{ t("common.name") }}
              <input v-model="adminCategoryForm.name" type="text" class="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm" :placeholder="t('admin.categoryPlaceholder')" />
            </label>
          </div>

          <div class="mt-5 flex flex-wrap gap-3">
            <button type="button" class="rounded-full bg-slate-950 px-5 py-2.5 text-sm font-medium text-white transition hover:bg-slate-800" @click="handleSaveAdminCategory">
              {{ adminCategoryEditingId ? t("common.update") : t("admin.createAction") }}
            </button>
          </div>
        </div>
        ...
        <div class="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.22)]">
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <h3 class="mt-2 text-lg font-semibold text-slate-950">{{ t("admin.existingCategories") }}</h3>
              <p class="mt-2 text-sm leading-6 text-slate-500">
                {{ t("admin.categoriesOverviewLead") }}
              </p>
            </div>
            <span class="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-sm font-medium text-slate-600">
              {{ t("admin.categoryCount", adminCategories.length) }}
            </span>
          </div>

          <ul class="mt-6 grid gap-3" data-testid="admin-category-list">
            <li v-for="category in adminCategories" :key="category.id" class="rounded-[1.35rem] border border-slate-200 bg-slate-50/70 p-4">
              <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <p class="text-sm font-semibold text-slate-900">{{ category.name }}</p>
                <div class="flex flex-wrap gap-2">
                  <button type="button" class="rounded-full border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-sky-200 hover:bg-sky-50/70" @click="startAdminCategoryEdit(category)">
                    {{ t("common.edit") }}
                  </button>
                  <button type="button" class="rounded-full bg-rose-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-rose-700" @click="handleDeleteAdminCategory(category.id)">
                    {{ t("common.delete") }}
                  </button>
                </div>
              </div>
...
const { adminError, adminLoading, adminCategoryEditingId, adminCategoryForm, adminCategories } = storeToRefs(adminStore);
const { resetAdminCategoryForm, handleSaveAdminCategory, startAdminCategoryEdit, handleDeleteAdminCategory } = adminStore;
</script>
